...
& Lam2,epYU,Yu,Yd,Ye,epYD,epYE,M12,M112,M222,vd,vu,gTAh,gThh,gTHm,gTVWm,gTVZ,            & 
& gFuFucFdFd,gFuFucFeFe,gFuFucFuFu,gFuFdcFeFv,gFuFucFvFv,epsI,deltaM,CheckRealStates,    & 
& gT,gPartial,BR)
...
& Lam4,Lam3,Lam2,epYU,Yu,Yd,Ye,epYD,epYE,M12,M112,M222,vd,vu,cplcFdFdAhL,cplcFdFdAhR,    & 
...
Real(dp),Intent(in) :: MFu(3),MFd(3),MVZ,MVWm,MHm(2),MAh,Mhh(2)
...
Real(dp),Intent(inout) :: gTVZ,gTVWm,gTHm(2),gTAh,gThh(2)
...
coup(2) = Conjg(cplcFuFuAhL(iIN,gt1)) 
coup(1) = Conjg(cplcFuFuAhR(iIN,gt1)) 
...
coup2(1) = cplcFuFuVZL(iIN,gt1) 
coup2(2) = cplcFuFuVZR(iIN,gt1) 
coup2(3) = Conjg(cplcFuFuAhL(iIN,gt1)) 
coup2(4) = Conjg(cplcFuFuAhR(iIN,gt1))  
coup2(5) = cplcFdFdVZL(gt2,gt3) 
coup2(6) = cplcFdFdVZR(gt2,gt3) 
...
coup2(1) = cplcFuFuAhL(iIN,gt1) 
coup2(2) = cplcFuFuAhR(iIN,gt1) 
coup2(3) = Conjg(cplcFuFuhhL(iIN,gt1,i2)) 
coup2(4) = Conjg(cplcFuFuhhR(iIN,gt1,i2))  
...
Real(dp),Intent(in) :: MFu(3),MFe(3),MVZ,MAh,Mhh(2)
...
Real(dp),Intent(in) :: MFu(3),MVZ,MAh,Mhh(2)

Complex(dp),Intent(in) :: cplcFuFuAhL(3,3),cplcFuFuAhR(3,3),cplcFuFuhhL(3,3,2),cplcFuFuhhR(3,3,2),              & 
...
coup(2) = Conjg(cplcFuFuAhL(iIN,gt3)) 
coup(1) = Conjg(cplcFuFuAhR(iIN,gt3)) 
coup(4) = Conjg(cplcFuFuAhL(gt2,gt1)) 
coup(3) = Conjg(cplcFuFuAhR(gt2,gt1))
Call IntegrateScalarSS(Boson2,mass,coup,deltaM,epsI,IntegralSs,NSs,resR, check) 
If (gt1.Eq.gt3) Then 
resR=resR/2._dp 
End If
...
coup2(1) = cplcFuFuAhL(iIN,gt3) 
coup2(2) = cplcFuFuAhR(iIN,gt3) 
coup2(3) = Conjg(cplcFuFuAhL(iIN,gt1)) 
coup2(4) = Conjg(cplcFuFuAhR(iIN,gt1))  
coup2(5) = cplcFuFuAhL(gt2,gt1) 
coup2(6) = cplcFuFuAhR(gt2,gt1) 
...
coup2(1) = cplcFuFuVZL(iIN,gt3) 
coup2(2) = cplcFuFuVZR(iIN,gt3) 
coup2(3) = Conjg(cplcFuFuAhL(iIN,gt3)) 
coup2(4) = Conjg(cplcFuFuAhR(iIN,gt3))  
coup2(5) = cplcFuFuVZL(gt2,gt1) 
coup2(6) = cplcFuFuVZR(gt2,gt1) 
coup2(7) = Conjg(cplcFuFuAhL(gt2,gt1)) 
coup2(8) = Conjg(cplcFuFuAhR(gt2,gt1)) 
Call IntegrateGaugeSscalarS(Boson4, mass, coup2, deltaM, epsI,IntegralVSss,NVSss, resC, check) 
If (resC.ne.resC) resC = 0._dp
resC = 2._dp*resC 
resC= 3*resC ! color factor 
...
End Module Fu3Decays_THDMIIIHB 
