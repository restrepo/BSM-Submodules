...
Module LoopCouplings_SimplifiedDMSSSFDM 
...
Real(dp) Function AlphaEwDR(Q,MHp,MVWp,MFre,MFd,MFu,MFe) 
 
Real(dp),Intent(in)::Q,MHp,MVWp,MFre,MFd(3),MFu(3),MFe(3)
Integer::i1 
Real(dp)::DeltaAlpha 
If (MZ_input) then 
...
End if 
If (MFre.gt.Q) Then 
DeltaAlpha=DeltaAlpha-4._dp/3._dp*Log(MFre/ Q)*Alpha/(2._dp*pi) 
End if 
Do i1=1,3
If (MFd(i1).gt.Q) Then 
...
End if 
If (MFre.gt.Q) Then 
DeltaAlpha=DeltaAlpha+4._dp/3._dp*Log(MFre/ Q)
End if 
Do i1=1,3
If (MFd(i1).gt.Q) Then 
...
Real(dp) Function AlphaEW_T(AlphaEW_In, Q,MHp,MVWp,MFre,MFd,MFu,MFe) 
 
Real(dp),Intent(in)::AlphaEW_In, Q,MHp,MVWp,MFre,MFd(3),MFu(3),MFe(3)
Integer::i1 
Real(dp)::DeltaAlpha 
...
Subroutine DeltaVB(sinW2,sinW2_dr,rho,MAh,MFe,MFre,Mhh,MHp,Mss,ZER,ZEL,               & 
& Ye,Ys,res)

Implicit None 
Real(dp),Intent(in) :: MAh,MFe(3),MFre,Mhh,MHp,Mss,Ys(3)

Complex(dp),Intent(in) :: ZER(3,3),ZEL(3,3),Ye(3,3)

Real(dp) :: MAh2,MFe2(3),MFre2,Mhh2,MHp2,Mss2

Complex(dp) :: cplcFeFeAhL(3,3),cplcFeFeAhR(3,3),cplcFeFehhL(3,3),cplcFeFehhR(3,3),cplcFeFvcHpL(3,3),& 
& cplcFeFvcHpR(3,3),cplcFreFessL(3),cplcFreFessR(3),cplcFvFeHpL(3,3),cplcFvFeHpR(3,3)
...
chargefactor = 1 
If ((Include_in_loopss).and.(Include_in_loopFre).and.IncludeWave) Then 
If ((Mss2.gt.mf_l2(2)).Or.(MFre2.gt.mf_l2(2))) Then
coup1L = cplcFreFessL(gt1)
coup1R = cplcFreFessR(gt1)
coup2R = Conjg(cplcFreFessL(gt2))
coup2L = Conjg(cplcFreFessR(gt2))
sumI = sumI + chargefactor*0.5_dp*coup1L*coup2R*B1(0._dp,MFre2,Mss2)  
End if 
End if 
...
End Module LoopCouplings_SimplifiedDMSSSFDM 
